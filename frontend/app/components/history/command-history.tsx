...
function CommandItemRow({
  ...
  const { activeChatId: chatId } = useProjectStore()
...
export function CommandHistory({
  ...
  if (isOpen && !hasPrefetchedRef.current) {
    ...
    recentChats.forEach((chat) => {
      if (chat.projectId) {
        router.prefetch(`/project/${chat.projectId}`)
      }
    })
      ...
      return (
        <CommandItem
          ...
          onSelect={() => {
            ...
            if (!editingId && !deletingId && chat.projectId) {
              // Navigate to the project and set the active chat
              const { setActiveChatId } = useProjectStore.getState()
              setActiveChatId(chat.id)
              router.push(`/project/${chat.projectId}`)
            }
          }}
          className={cn(
